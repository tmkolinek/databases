...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:luvezx123@localhost:5432/linksoft'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SQLALCHEMY_ECHO'] = True
...
def add_playlist():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(playlist)
        db.session.commit()
        return redirect(url_for('playlists', add_playlist=form))#redirect("playlists.html")
    ...
    # ADD THE NECESSARY CODE HERE FOR THIS ROUTE TO WORK
        ...
        return redirect(url_for('songs.html', song=form))
        # return redirect("songs.html")
    else:
        return render_template('new_song.html', form=form)#redirect('new_song.html', form=form)
